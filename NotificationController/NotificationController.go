...
func AdminNotificationController(ctx *gin.Context) {
	...
	for rows.Next() {
		...
		go telegrambot.SendMessageFromMainBot(escapeMessage, telegramUser, messageChatID)
	}
}
...
func HealthBotController(ctx *gin.Context) {
	admin := "Admin"
	...
	for rows.Next() {
		...
		go telegrambot.SendMessageFromHealthBot(escapeMessage, telegramUser, messageChatID)
	}
}
